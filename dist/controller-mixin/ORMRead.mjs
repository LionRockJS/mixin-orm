...
export default class ControllerMixinORMRead extends ControllerMixin {
    ...
    static init(state) {
        ...
        if (!state.get(this.DATABASE_KEY))
            state.set(this.DATABASE_KEY, 'admin');
        if (!state.get(this.MODEL))
            state.set(this.MODEL, state.get(this.MODEL));
        if (!state.get(this.LIST_FILTER))
            state.set(this.LIST_FILTER, []);
    ...
    static verfiy_order_by(state) {
        ...
        [...orderBy.keys()].forEach(it => {
            ...
            if (parts[0] === 'id' || parts[0] === 'created_at' || parts[0] === 'updated_at')
                return;
