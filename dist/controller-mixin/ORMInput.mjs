import { ControllerMixin, ControllerState, ORM } from '@lionrockjs/central';
...
const parseClassField = async (result, postData, Model, currentID = '?') => {
    ...
    await Promise.all([...postData.entries()].map(async (it) => {
        ...
        if (Array.isArray(v)) {
            ...
            if (!Model.belongsToMany.has(prop) && !Type.belongsToMany.has(type))
                throw new Error(`Invalid input siblings ${prop} for ${Model.name}`);
        }
...
const parseChildField = async (result, postData, Model, currentID = '?') => {
    ...
    await Promise.all([...postData.entries()].map(async (it) => {
        ...
        if (id !== '?' && parentID === '?')
            throw new Error(`${type}(${id}) cannot own by new ${Model.name}() `);
        const fk = parseFK(matches[4], `${Model.joinTablePrefix}_id`);
        const prop = matches[5];
        const value = Array.isArray(v) ? v.map(x => parseInt(x, 10)) : v;
        const Type = await ORM.import(type);
        if (!(Type.fields.has(prop) || Type.belongsTo.has(prop)))
            throw new Error(`Invalid input field ${prop} for ${Type.name}`);
        if (!(Type.belongsTo.has(fk) || Type.hasMany.map((x) => x[1]).includes(Model.name)))
            throw new Error(`Invalid FK ${fk} for ${Type.name}`);
        const instancesType = mapGetOrCreate(result, type, new Map());
...
const parseChainChild = async (result, postData, Model, currentID = '?') => {
    ...
    await Promise.all([...postData.entries()].map(async (it) => {
        ...
        types.forEach((m, i) => {
            ...
            if (parentType.belongsTo.has(m.name))
                throw new Error(`${m.name} not belongs to ${parentType.name}`);
        });
