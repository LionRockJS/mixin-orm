...
export default class ControllerMixinORMWrite extends ControllerMixin {
    ...
    static init(state) {
        if (!state.get(ControllerMixinORMWrite.DATABASE_KEY))
    ...
    static async #newInstance(input, model, orm_options) {
        ...
        // add many to many
        await Promise.all(Array.from(mapNewInstance.entries()).map(async (v) => {
            const key = v[0];
            if (!/^[~:]/.test(key))
                return;
    ...
    static async #updateInstances(input, id, orm_options, state) {
        await Promise.all(Array.from(input.entries()).map(async (x) => {
            ...
            await Promise.all(results.map(async (model) => {
                const m = model;
                ...
                if (changed) {
                    ...
                    await instance.write();
                    const manyOps = [];
                    await Promise.all([...MClass.belongsToMany].map(async (xx) => {
                        const k = `~${xx}`;
                        if (m[k] === undefined)
                            return;
                        if (!Array.isArray(m[k]))
                            throw new Error(`${k} must be Array`);
                        const ModelB = await ORM.import(k.slice(1));
                        const many = m[k].filter((nid) => nid !== 'replace').map((nid) => new ModelB(nid, orm_options));
                        if (many.length !== m[k].length)
                            manyOps.push(instance.removeAll(ModelB));
                        if (many.length > 0)
                            manyOps.push(instance.add(many));
                    }));
