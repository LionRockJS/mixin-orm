...
const parseModelId = (code: string, currentID: number | string) => {
  if (!code || code === '()') return currentID;
  return parseInt(code.replace(/[^0-9]/g, ''), 10);
};

const parseFK = (code: string, defaultFK: any) => {
  if (!code || code === '-') return defaultFK;
  return code.replace(/^-/, '');
};

const parseClassField = async (result: Map<any, any>, postData: Map<any, any>, Model: any, currentID: any = '?') => {
  const pattern = /^(\(\d+\))?:(\w+)$/;
...
const parseChildField = async (result: Map<any, any>, postData: Map<any, any>, Model: any, currentID: any = '?') => {
  ...
  [...postData.entries()].map(async (it) => {
    ...
    if (!(Type.belongsTo.has(fk) || Type.hasMany.map((x: any)=>x[1]).includes(Model.name))) throw new Error(`Invalid FK ${fk} for ${Type.name}`);
...
const parseAddSibling = async (result: Map<any, any>, postData: Map<any, any>, Model: any, currentID: any = '?') => {
  const pattern = /^(\(\d+\))?~(\w+)([\[(][\])])?$/;
  await Promise.all([...postData.entries()].map(async (it) => {
    const key = it[0];
    const v = it[1];
...
const parseChainChild = async (result: Map<any, any>, postData: Map<any, any>, Model: any, currentID: any = '?') => {
  ...
  [...postData.entries()].map(async (it) => {
    const v = it[1];
    const key = it[0];
    const matches = pattern.exec(key as string);
    if (!matches) return;

    const types = await Promise.all([matches[3], matches[6], matches[9], matches[12], matches[15]].filter(x => !!x).map(async x => await ORM.import(x)));
    ...
    const value = Array.isArray(v) ? v.map((x: any) => parseInt(x, 10)) : v;
    if (!Type.fields.has(prop) && !Type.belongsTo.has(prop)) throw new Error(`Invalid input field ${prop} for ${Type.name}`);
