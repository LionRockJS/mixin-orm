...
export default class ControllerMixinORMWrite extends ControllerMixin {
  ...
  static async #newInstance(input: Map<string, any>, model: any, orm_options: any) {
    ...
    // add many to many
    await Promise.all(
      Array.from(mapNewInstance.entries()).map(async (v: any) => {
        const key = v[0];
        if (!/^[~:]/.test(key)) return;

        const ModelB = await ORM.import(key.slice(1));
        const models = v[1].filter((id: any) => id !== 'replace').map((id: any) => new ModelB(id, orm_options));
  ...
  static async #updateInstances(input: Map<string, any>, id: any, orm_options: any, state: Map<string, any>) {
    await Promise.all(
      Array.from(input.entries()).map(async x => {
        ...
        await Promise.all(
          results.map(async (model: any) => {
            ...
            MClass.fields.forEach((xx: any, field: string) => {
            ...
            MClass.belongsTo.forEach((xx: any, field: string) => {
              const v = newValues.get(field);
            ...
            MClass.belongsToMany.forEach((field: string) => {
              const k = `~${field}`;
            ...
            if (changed) {
              ...
              await Promise.all(
                ...
                [...MClass.belongsToMany].map(async xx => {
                  ...
                  const many = m[k].filter((nid: any) => nid !== 'replace').map((nid: any) => new ModelB(nid, orm_options));
  ...
  static async #newChild(input: Map<string, any>, parent: any, orm_options: any) {
    await Promise.all(
      Array.from(input.entries()).map(async x => {
        const list = x[1];
        const type = x[0];
        ...
        MClass.belongsTo.forEach((ParentModel: any, fk: string) => {
